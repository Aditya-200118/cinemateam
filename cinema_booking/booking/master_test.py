...
def create_dummy_data():
    ...
    for title, category, cast, director, producer, synopsis, trailer_url, rating, release_date, poster in movies_data:
        ...
        MovieTicketTypeDiscount.objects.create(movie=movie, ticket_type=ticket_types[0], discount=15.0)  # Child discount
        MovieTicketTypeDiscount.objects.create(movie=movie, ticket_type=ticket_types[1], discount=30.0)  # Senior discount
        MovieTicketTypeDiscount.objects.create(movie=movie, ticket_type=ticket_types[2], discount=0.0)   # Adult discount
